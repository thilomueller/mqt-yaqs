...
def test_physics_simparams_basic() -> None:
    ...
    observables, elapsed_time, dt, times array, sample_timesteps flag, and N are set correctly.
    """
    obs_list = [Observable("x", 0)]
    elapsed_time = 1.0
    dt = 0.2
    params = PhysicsSimParams(obs_list, elapsed_time, dt=dt, sample_timesteps=True, N=50)
...
def test_observable_initialize_with_sample_timesteps() -> None:
    ...
    """
    obs = Observable("x", 1)
    sim_params = PhysicsSimParams([obs], elapsed_time=1.0, dt=0.5, sample_timesteps=True, N=10)
...
def test_observable_initialize_without_sample_timesteps() -> None:
    ...
    has shape (N, 1), and that the observable's times attribute is set to elapsed_time.
    """
    obs = Observable("x", 0)
    sim_params = PhysicsSimParams([obs], elapsed_time=1.0, dt=0.25, sample_timesteps=False, N=5)
