...
im3 = axes[3].imshow(np.abs(heatmap_exact-heatmap1000), cmap='Reds', aspect='auto', extent=[0, data['sim_params'].T, L, 0], norm=LogNorm(vmin=1e-3, vmax=0.1), interpolation='none')
...
for site in range(L):
    ...
    for _ in range(num_samples):
        ...
        indices = np.random.choice(data['sim_params'].observables[site].trajectories.shape[0], trajectories, replace=True)
...
im1 = axes[1].imshow(np.abs(heatmap_exact-heatmap10), cmap='Reds', aspect='auto', extent=[0, data['sim_params'].T, L, 0], norm=LogNorm(vmin=1e-3, vmax=0.1), interpolation='none')
...
im2 = axes[2].imshow(np.abs(heatmap_exact-heatmap100), cmap='Reds', aspect='auto', extent=[0, data['sim_params'].T, L, 0], norm=LogNorm(vmin=1e-3, vmax=0.1), interpolation='none')

cbar = plt.colorbar(im3, use_gridspec=True)
# cbar = plt.colorbar(im, ax=axes[3], aspect=10)
