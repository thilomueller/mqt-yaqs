...
D_param = J_z = 1  # Z coupling strength
h = 0.5  # transverse field strength
gamma_dephasing = 0.1 #1 / 10.0  # dephasing rate (1/T2star)
gamma_relaxation = 0.1 #1 / 10.0  # relaxation rate (1/T1)
...
# # Initial state (first spin down, rest up)
# # psi0_mcwf = qt.tensor([qt.basis(2, 1)] + [qt.basis(2, 0) for _ in range(N-1)])
# # # Initial state (all spins up)
# # psi0_mcwf = qt.tensor([qt.basis(2, 0) for _ in range(N)])
# # Initial state (up,down,up,down...)
# psi0_mcwf = qt.tensor([qt.basis(2, i % 2) for i in range(N)])
# ntraj = 100  # number of trajectories
# options = qt.Options(store_states=True)  # Ensure states are stored
# result = qt.mcsolve(H, psi0_mcwf, t, c_ops, e_ops, ntraj=ntraj, options=options)

#endregion

# ''' Lindblad MPO solution'''
...
#     "init_product_state": ["-z"]+ ["+z"]*(N-1),  # initial state 
...
# x_expectation_values_mpo = np.array([[solver.result['obs-1q'][('x', (i,))][1][t] 
#                                 for t in range(len(solver.result['obs-1q'][('x', (i,))][0]))] for i in range(N)])
# y_expectation_values_mpo = np.array([[solver.result['obs-1q'][('y', (i,))][1][t] 
...
# Restructure exp_vals_lindbladmpo to match desired format
exp_vals_lindbladmpo = []



# Loop over each qubit and each observable to append in the correct order


# for component in ['x', 'y', 'z']:
#     for qubit_index in range(N):
#         exp_vals_lindbladmpo.append(solver.result['obs-1q'][(component, (qubit_index,))][1])
...
D = create_D(N)
mpoH = ptn.heisenberg_xxz_mpo(N, J=-4*J, D=-4*D_param, h=2*h)
mpoH.zero_qnumbers()
...
operator_site = 2  # Measure the first spin
dt = T/timesteps

# Run the TN-MCWF simulation
num_trajectories = 30  # Adjust as needed
